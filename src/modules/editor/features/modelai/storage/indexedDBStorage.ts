...
export class IndexedDBStorage implements IStorage {
  ...
  async get(database: string, table: string, id: string): Promise<any> {
    ...
    return IndexedDBStorage.get(db, table, id).finally(() => {
      db.close();
    });
  }
  ...
  async put(
    ...
    const db = await this.open(database);
    return IndexedDBStorage.put(db, table, id, value).finally(() => {
      db.close();
    });
  }
  ...
  async delete(database: string, table: string, id: string): Promise<boolean> {
    const db = await this.open(database);
    return IndexedDBStorage.delete(db, table, id).finally(() => {
      db.close();
    });
  }
  ...
  async page(database: string, table: string, page: number): Promise<any[]> {
    const db = await this.open(database);
    return IndexedDBStorage.getPage(db, table, page).finally(() => {
      db.close();
    });
  }
